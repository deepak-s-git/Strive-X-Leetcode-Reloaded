...
// - For mid = (left + right)/2, count how many nums <= mid.
// - If count > mid, duplicate is in [left, mid]; else in [mid+1, right].
// - Works without modifying array and with O(1) extra space.
//
// Time Complexity: O(n log n)   (O(log n) iterations × O(n) count step)
// Space Complexity: O(1)
//
class Solution {
public:
